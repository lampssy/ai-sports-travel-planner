...
short-lived exact-view handoff, not durable search-session persistence or a
portable snapshot of every catalog and trust input.
...
canonical `applied_intent` and stores a typed, lightweight evaluated-baseline
snapshot in the web process.

The snapshot contains only the canonical intent SHA-256 digest, the public
baseline fingerprint, and the minimum evaluated candidate, ranking, model,
policy, and evidence-selection state needed to validate proposals and build
deterministic previews against the ranking that was delivered. It does not
retain the full `SearchIntent` or origin text, a full `CatalogSnapshot` or
`CatalogTrustManifest`, the trip brief, provider credentials, prompts, tokens,
responses, or other provider secrets.
...
validates optional refinement proposals from that snapshot and never reruns
...
- Advisory review: exact-state feature review completed on 2026-07-17 with no
  remaining Blocker, High, Medium, or Low findings.
...
Refinement proposals and previews use the exact evaluated baseline behind the
ranking the user sees. Achieving that consistency intentionally retains typed
evaluated-baseline data in process for up to 60 seconds and adds bounded memory,
expiry, eviction, and concurrency behavior that must be tested and observed.
The retained value is narrower than the catalog and trust inputs used to create
it.
...
- Store the full catalog/trust snapshot, persist the evaluated baseline, or put
  it in a shared cache: supports a longer or multi-instance handoff, but retains
  more data and adds invalidation, serialization, compatibility, and operational
  ownership that the current single-instance product does not require.
