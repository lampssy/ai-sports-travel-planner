...
| `validate_boundary_adjudication` | keep a `policy_determined` ski-area promotion only when the returned separate/folded IDs match the review output; on failure, do not authorize a separate ski-area fix. Fold only into a parent assessed as a separate ski area in the same adjudication. Before terminal `evidence_insufficient` for adjacent candidates, return to graph discovery when a plausible evidence-backed connected-cluster parent was not assessed |
