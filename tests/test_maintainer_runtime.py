...
def test_simple_run_lease_has_minimal_private_owner_payload(tmp_path: Path) -> None:
...
def test_simple_run_lease_blocks_fresh_competitor(tmp_path: Path) -> None:
    lease = SimpleRunLease.acquire(tmp_path, "curation", now=NOW)

    with pytest.raises(LockBusyError, match="curation"):
        SimpleRunLease.acquire(
            tmp_path,
            "discovery",
            now=NOW + timedelta(minutes=1),
        )

    assert SimpleRunLease.load_owner(tmp_path, "curation", lease.run_id) == lease


def test_simple_run_lease_preserves_stale_lock_on_takeover(tmp_path: Path) -> None:
    ...
    assert SimpleRunLease.load_owner(tmp_path, "discovery", new.run_id) == new


def test_simple_old_same_worker_run_id_cannot_adopt_stale_successor(
    tmp_path: Path,
) -> None:
    old = SimpleRunLease.acquire(tmp_path, "curation", now=NOW)
    new = SimpleRunLease.acquire(
...
def test_simple_other_worker_cannot_use_active_run_id(tmp_path: Path) -> None:
...
def test_simple_heartbeat_and_release_require_exact_owner(tmp_path: Path) -> None:
    lease = SimpleRunLease.acquire(tmp_path, "curation", now=NOW)
    later = NOW + timedelta(minutes=5)

    loaded = SimpleRunLease.load_owner(tmp_path, "curation", lease.run_id)
    ...
    assert _read_json(lease.owner_path) == {
        ...
        "run_id": lease.run_id,
        "acquired_at": NOW.isoformat(),
        "heartbeat_at": later.isoformat(),
    }
    forged = SimpleRunLease(
        state_dir=tmp_path,
        worker="curation",
        run_id="f" * 32,
    )
...
@pytest.mark.parametrize(
    "unsafe_kind",
    ["symlink", "non-regular", "malformed", "oversized"],
)
def test_simple_owner_rejects_unsafe_or_invalid_metadata(
    tmp_path: Path,
    unsafe_kind: str,
) -> None:
    lease = SimpleRunLease.acquire(tmp_path, "curation", now=NOW)
    ...
    if unsafe_kind not in {"symlink", "non-regular"}:
...
def test_simple_run_lease_rejects_state_directory_symlink(tmp_path: Path) -> None:
    state_dir = tmp_path / "state"
    SimpleRunLease.acquire(state_dir, "curation", now=NOW)
    alias = tmp_path / "state-alias"
    alias.symlink_to(state_dir, target_is_directory=True)

    with pytest.raises(RunLeaseError):
        SimpleRunLease.load_owner(alias, "curation", "0" * 32)
