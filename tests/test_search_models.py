...
def test_search_model_selection_defaults_to_v1(monkeypatch) -> None:
    ...
    assert selection.configured_search_model == "search_v1"
    assert selection.requested_search_model is None
    assert selection.effective_search_model == "search_v1"
...
def test_search_model_selection_rejects_unknown_model(monkeypatch) -> None:
    monkeypatch.setenv("SNOWCAST_SEARCH_MODEL", "search_v3")
